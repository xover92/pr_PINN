...
class PINN_1d(nn.Module):
    ...
    Attributes:
    hidden (nn.Sequential): the neural network architecture itself, which is composed by its 4 layers and their Tanh activation functions.
    """
    def __init__(self):
...
def pde_residual_1d(x: torch.Tensor, t: torch.Tensor, model: nn.Module, D: float=0.1, R: float=1) -> torch.Tensor:
    """
    ...
    u_xx=torch.autograd.grad(u_x, x, torch.ones_like(u), create_graph=True)[0] #d^2u/dx^2

    residual = u_t-D*u_xx-R*u*(1-u)
    return residual

def exact_solution_1D(x:torch.Tensor, t:torch.Tensor, D:float=0.1, R:float=1) -> torch.Tensor:
    """
...
def loss_function_1d(x:torch.Tensor, t:torch.Tensor, model:nn.Module) -> float:
    """
    Computes the loss function as the sum of the initial conditions loss function, boundary conditions loss function and the residual loss function.
