...
def loss_function_2d(x: torch.Tensor,
    ...
    x_train, y_train, t_train = torch.meshgrid(
    ...
    # residual loss
    loss_pde = torch.mean(pde_residual_2d(x_train, y_train, t_train, model)**2)

    # total loss
    return loss_bc+loss_ic+loss_pde


def training_loop_2D(n_epochs: int, n_neurons: int) -> nn.Module:
    """
...
def generate_plot_2d(n_epocs: int, n_neurons: int) -> Figure:
    """
    ...
    model = training_loop_2D(n_epocs, n_neurons)
